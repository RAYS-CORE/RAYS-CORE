...
class InstagramValidator(BaseValidator):
    def validate(self, url: str, html: str, dom: dict | None = None) -> ValidationResult:
        ...
        if matched_neg:
            return self._not_found(f"Instagram: {matched_neg[0]}", signals=matched_neg)

        matched_priv = self._check_patterns(html, self.rules.get("private_indicators", []))
        if matched_priv:
            return self._private(f"Instagram: {matched_priv[0]}", signals=matched_priv)

        matched_login = self._check_patterns(html, self.rules.get("login_indicators", []))
        if matched_login:
            return self._login_required("Instagram: Login required", signals=matched_login)
        ...
        if matched_pos:
            ...
            return self._found(f"Instagram profile found: {len(matched_pos)} positive signals",
                               confidence=confidence, signals=signals)

        return self._not_found("Instagram: No recognizable profile signals", signals=signals)
