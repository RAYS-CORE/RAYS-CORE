...
class MediumValidator(BaseValidator):
    def validate(self, url: str, html: str, dom: dict | None = None) -> ValidationResult:
        ...
        if matched_neg:
            return self._not_found(f"Medium: {matched_neg[0]}", signals=matched_neg)

        matched_login = self._check_patterns(html, self.rules.get("login_indicators", []))
        if matched_login:
            return self._login_required("Medium: Login required", signals=matched_login)
        ...
        if matched_pos:
            ...
            return self._found(f"Medium profile found: {len(matched_pos)} positive signals",
                               confidence=confidence, signals=signals)

        return self._not_found("Medium: No recognizable profile signals", signals=signals)
