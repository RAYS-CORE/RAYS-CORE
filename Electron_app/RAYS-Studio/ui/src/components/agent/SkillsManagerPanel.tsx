...
export function SkillsManagerPanel({ open, onClose, workspaceRoot }: SkillsManagerPanelProps) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const skill of filteredSkills) {
      const cat = skill.category || getCategory(skill.name);
                                  ...
                                  {skill.platforms && skill.platforms.length > 0 && (
                                    <div className="flex flex-wrap gap-1 justify-end mt-1">
                                      {skill.platforms.map((p) => (
                                        <span key={p} className="text-[8px] px-1 py-0.5 rounded border border-white/10 bg-white/5 text-muted-foreground uppercase">
                                          {p}
                                        </span>
                                      ))}
                                    </div>
