...
class ImageGenerationService:
    ...
    def get_image_gen_func(self):
        ...
        elif is_rays_studio_selected():
            return self.generate_image_rays_studio
        return self.generate_image_rays_studio
    ...
    async def generate_image(self, prompt: ImagePrompt) -> str | ImageAsset:
        ...
        if not self.image_gen_func:
            print("No image generation function found. Falling back to RAYS Studio.")
            self.image_gen_func = self.generate_image_rays_studio
    ...
    async def generate_image_rays_studio(self, prompt: str, output_directory: str) -> str:
                    ...
                    if resp.status == 200:
                        ...
                        if choices:
                            ...
                            if message.get("type") == "image":
                                base64_data = message.get("base64", "").replace("data:image/png;base64,", "")
                                image_path = os.path.join(output_directory, f"{uuid.uuid4()}.png")
                                with open(image_path, "wb") as f:
                                    f.write(base64.b64decode(base64_data))
                                return image_path
        except Exception as e:
            print(f"Error querying RAYS Studio for image: {e}")
        return "/static/images/placeholder.jpg"
