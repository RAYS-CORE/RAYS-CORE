...
export const PresentationGrid = ({
  ...
  const ShimmerCard = () => (
    <div className="flex flex-col gap-4 min-h-[200px] bg-card/70 rounded-lg p-4 animate-pulse">
  ...
  const CreateNewCard = () => (
    <div
      ...
      className="flex flex-col cursor-pointer group ring-1 ring-inset ring-border hover:ring-primary/40 bg-card/80 rounded-xl overflow-hidden transition-all duration-300 font-syne"
    >
      <img src="/create_presentation.png" alt="New Presentation" className="w-full aspect-[16/11] object-cover" />
      <div className="flex items-center gap-3 p-3 mt-auto  border border-border">
        ...
        <div>
          <h4 className="text-sm text-foreground font-semibold tracking-[0.14px]">Create New Presentation</h4>
          <p className="text-sm text-muted-foreground font-medium tracking-[0.14px] flex items-center gap-2">Get Started <ArrowRight className="w-4 h-4" /></p>
        </div>
      </div>
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 px-6 mt-10 md:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6 w-full">
        <div className="flex flex-col gap-4 min-h-[200px] cursor-pointer group ring-1 ring-inset ring-border bg-card/80 rounded-xl items-center justify-center animate-pulse">
