...
const conversationStorageKey = (presentationId: string) =>
  `deckforge:chat:conversationId:${presentationId}`;

const AssistantMarker = () => (
  <div className="mb-3 flex items-center gap-1.5 text-[#888888]">
...
const Chat = ({
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#1d1d1d]">
      <div className="flex items-center justify-between px-4 pt-8">
        <div className="flex items-center gap-2">
          <h4 className="flex items-center gap-2 text-sm font-semibold text-[#ffffff]">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
          ...
          {isSending && (
            <span className="inline-flex items-center gap-1 rounded-[10px] bg-[#383838] px-2 py-0.5 text-[10px] font-medium text-[#ffffff]">
        ...
        <button
          ...
          className="rounded-[10px] p-1 text-[#888888] transition-colors hover:bg-[#383838] hover:text-[#ffffff] disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4 pt-9 hide-scrollbar">
        {isHistoryLoading && messages.length === 0 ? (
          ...
          <>
            <div>
              <h4 className="mb-2 text-[10px] font-normal leading-[15px] tracking-[0.367px] text-[#888888]">
                SUGGESTIONS
              </h4>
              <div className="flex flex-col gap-1.5">
                {suggestions.map((suggestion) => (
                  <button
                    ...
                    className="flex cursor-pointer items-center gap-3 rounded-[10px] border border-[#383838] px-3 py-2 text-left transition-colors hover:bg-[#383838]"
                  >
                    {suggestion.icon}
                    <span className="text-xs font-normal leading-[15px] tracking-[0.367px] text-[#ffffff]">
              ...
              <div className="flex flex-wrap gap-2">
                {quickPrompts.map((prompt) => (
                  <button
                    ...
                    className="cursor-pointer rounded-[10px] border border-[#383838] px-2.5 py-1 transition-colors hover:bg-[#383838]"
                  >
                    <span className="text-xs font-normal leading-[15px] tracking-[0.367px] text-[#ffffff]">
                      {prompt}
                    </span>
          ...
          <div className="flex flex-col gap-9">
            {messages.map((message) =>
              message.role === "user" ? (
                ...
                >
                  <div className="max-w-[78%] rounded-[10px] bg-[#383838] px-4 py-3 text-sm font-medium leading-5 text-[#ffffff]">
                    <p className="whitespace-pre-wrap">{message.content}</p>
                  </div>
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[#ffffff] text-sm font-semibold text-[#000000]">
                    U
                  </div>
                </div>
              ) : (
                <div key={message.id} className="max-w-[92%]">
                  ...
                  {message.content ? (
                    message.role === "error" ? (
                      <div className="whitespace-pre-wrap text-sm font-normal leading-5 text-red-500">
                        {message.content}
                      </div>
                    ) : (
                      <div className="chat-markdown mb-0 text-sm font-normal leading-5 text-[#ffffff]">
                        <MarkdownRenderer
                          content={message.content}
                          className="chat-markdown mb-0 text-sm font-normal leading-5 text-[#ffffff]"
                        />
                        {isSending && message.id === activeAssistantMessageId && (
                          <span
                            aria-hidden="true"
                            className="ml-1 inline-block h-4 w-0.5 animate-pulse rounded-[10px] bg-[#ffffff] align-middle"
                          />
                        )}
                      </div>
                    )
                  ) : (
                    <div className="text-sm font-normal leading-5 text-[#ffffff]">
                  ...
                  {message.activity && message.activity.length > 0 && (
                    <div className="mt-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-left text-xs font-medium text-[#888888] hover:text-[#ffffff]"
                      >
                        ...
                        {message.activity.some((item) => item.state === "running") && (
                          <Loader2 className="h-3 w-3 animate-spin text-[#888888]" />
                        )}
                      </button>

                      {expandedActivityByMessage[message.id] && (
                        <div className="mt-2 space-y-1.5 pl-4 border-l border-[#383838]">
                          {message.activity.map((activityItem) => (
                            <div
                              key={activityItem.id}
                              className="text-xs leading-4 text-[#888888]"
                            >
                              {activityItem.tool && (
                                <span className="mr-1 text-[#ffffff]">
                          ...
                          {message.toolCalls && message.toolCalls.length > 0 && (
                            <div className="pt-0.5 text-[11px] text-[#888888]">
      ...
      <form
        ...
        className="relative mx-4 mb-4 rounded-[10px] border border-[#383838] bg-[#000000] px-2.5 py-3"
      >
        <textarea
          ref={inputRef}
          name="chat-input"
          id="chat-input"
          className="min-h-[92px] w-full resize-none bg-transparent pb-10 text-sm text-[#ffffff] placeholder:text-[#888888] focus:outline-none focus:ring-0"
          rows={4}
          value={input}
          disabled={isSending || isHistoryLoading}
        ...
        <button
          type="button"
          disabled
          className="absolute bottom-3 left-3 h-[28px] rounded-[10px] border border-[#383838] bg-[#1d1d1d] px-3 py-1 opacity-50"
        ...
        {isSending ? (
          <div className="absolute bottom-3 right-3 flex items-center gap-2">
            <button
              type="button"
              disabled
              className="flex cursor-wait items-center gap-1.5 rounded-[10px] border border-[#383838] bg-[#1d1d1d] px-3 py-2 text-sm font-medium text-[#ffffff]"
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-[10px] border border-[#383838] bg-[#1d1d1d] px-3 py-2 text-sm font-medium text-[#ffffff] transition-colors hover:bg-[#383838]"
          ...
          <button
            ...
            className="absolute bottom-3 right-3 flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-[#000000] bg-[#ffffff] disabled:cursor-not-allowed disabled:opacity-60 rounded-[10px]"
          >
            <Send className="h-3 w-3 text-[#000000]" />
            Send
          </button>
        )}
      </form>
