...
| Change | Upstream | RAYS-CORE |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `rays-core` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of RAYS-CORE infra |
...
| File operations | Bash commands | RAYS-CORE file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains RAYS-CORE-specific adaptations.
