...
## 5. RAYS-CORE Configuration

### Config location

`$RAYS_HOME/config.yaml` (defaults to `~/.rays-core/config.yaml` when `RAYS_HOME` is unset)
...
Restart the RAYS-CORE session for changes to take effect. The MCP connection is
