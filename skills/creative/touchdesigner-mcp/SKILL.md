...
metadata:
  rays-core:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, rays-core-video]
...
```
RAYS-CORE -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${RAYS_HOME:-$HOME/.rays-core}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to RAYS-CORE config (if missing)
...
3. **Restart RAYS-CORE session** to pick up the new MCP server
