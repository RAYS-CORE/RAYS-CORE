...
metadata:
  rays-core:
    ...
    related_skills: [claude-code, rays-core]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the RAYS-CORE terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For RAYS-CORE itself, `model.provider: openai-codex` uses RAYS-CORE-managed Codex
OAuth from `~/.rays-core/auth.json` after `rays-core auth add openai-codex`. For the
...
## RAYS-CORE Gateway Caveat

When invoking the Codex CLI from a RAYS-CORE gateway/service context (for example,
