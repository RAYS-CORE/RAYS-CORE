...
version: 1.1.1
author: xdevplatform + openclaw + RAYS-CORE
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by RAYS-CORE tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official RAYS-CORE Docker layout, `/opt/data` is `RAYS_HOME`, but RAYS-CORE tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for RAYS-CORE-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, RAYS-CORE tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the RAYS-CORE subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- RAYS-CORE adaptation: reformatted for RAYS-CORE skill conventions; safety guardrails preserved verbatim.
