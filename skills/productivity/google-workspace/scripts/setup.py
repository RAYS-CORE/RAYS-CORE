#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for RAYS-CORE.
...
# Ensure sibling modules (_rays_core_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _rays_core_home import display_rays_core_home, get_rays_core_home

RAYS_HOME = get_rays_core_home()
TOKEN_PATH = RAYS_HOME / "google_token.json"
CLIENT_SECRET_PATH = RAYS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = RAYS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same RAYS-CORE profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the RAYS-CORE Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the RAYS-CORE Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'rays-core[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to RAYS-CORE home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_rays_core_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for RAYS-CORE")
