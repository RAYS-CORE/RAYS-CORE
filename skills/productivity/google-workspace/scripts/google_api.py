#!/usr/bin/env python3
"""Google Workspace API CLI for RAYS-CORE.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing RAYS-CORE-facing JSON contract and falls back to the Python client
...
from _rays_core_home import get_rays_core_home
...
def _gws_binary() -> str | None:
    override = os.getenv("RAYS_CORE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for RAYS-CORE")
