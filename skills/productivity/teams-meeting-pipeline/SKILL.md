...
description: "Operate the Teams meeting summary pipeline via RAYS-CORE CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `rays-core teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${RAYS_HOME:-~/.rays-core}/.env`:
...
```bash
rays-core teams-pipeline validate              # config snapshot — run first after any change
rays-core teams-pipeline token-health          # Graph token status
rays-core teams-pipeline token-health --force-refresh   # force a fresh token acquisition
rays-core teams-pipeline list                  # recent meeting jobs
rays-core teams-pipeline list --status failed  # only failed jobs
rays-core teams-pipeline show <job-id>         # full detail of one job
rays-core teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
rays-core teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
rays-core teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
rays-core teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
rays-core teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

rays-core teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
rays-core teams-pipeline delete-subscription <sub-id>
rays-core teams-pipeline maintain-subscriptions            # renew near-expiry ones
rays-core teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `rays-core teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `rays-core cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
