#!/usr/bin/env bash
# GitHub environment detection helper for RAYS-CORE skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _rays_core_env="${RAYS_HOME:-$HOME/.rays-core}/.env"; [ -f "$_rays_core_env" ] && grep -q "^GITHUB_TOKEN=" "$_rays_core_env" 2>/dev/null; then
