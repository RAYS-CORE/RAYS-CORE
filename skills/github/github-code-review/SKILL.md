...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _rays_core_env="${RAYS_HOME:-$HOME/.rays-core}/.env"; [ -f "$_rays_core_env" ] && grep -q "^GITHUB_TOKEN=" "$_rays_core_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_rays_core_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from RAYS-CORE\",
    \"comments\": [
...
```bash
source "${RAYS_HOME:-$HOME/.rays-core}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by RAYS-CORE. Code looks clean — good test coverage, no security concerns."
