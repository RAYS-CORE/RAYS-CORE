"""
Cron job management tools for rays Agent.
...
from rays_constants import display_rays_home
...
#      saying "cat ~/.rays/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `rays-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.rays/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("rays_SESSION_PLATFORM")
    origin_chat_id = get_session_env("rays_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("rays_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes rays_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``rays_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via rays model.
    ...
    if provider_name == "custom":
        try:
            from rays_cli.runtime_provider import has_named_custom_provider
...
def _validate_cron_base_url(
    ...
    try:
        from rays_cli.runtime_provider import (
            has_named_custom_provider,
            resolve_requested_provider,
            _get_named_custom_provider,
        )
        from rays_cli.auth import PROVIDER_REGISTRY
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within rays_HOME/scripts/.
    ...
    from rays_constants import get_rays_home

    raw = script.strip()

    # Reject absolute paths and ~ expansion at the API boundary.
    # Only relative paths within ~/.rays/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.rays/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.rays/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_rays_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_rays_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("rays_INTERACTIVE")
        or env_var_enabled("rays_GATEWAY_SESSION")
        or env_var_enabled("rays_EXEC_ASK")
    )
