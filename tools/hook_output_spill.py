...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <rays_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  rays_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_rays_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: rays_HOME env var, then ~/.rays
            home = os.environ.get("rays_HOME") or os.path.expanduser("~/.rays")
